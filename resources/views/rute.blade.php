...
<html lang="en" style="background-color:darkslategray; color:antiquewhite;">
    ...
    <body> 
            ...
            @for($n=0;$n<count($gabung);$n++)
                @if($gabung[$n][7]=="start")
                    <svg id="pinstart" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="-0.5 0 25 25" fill="green" style="position: absolute; top:{{$gabung[$n][2]-30}}px; left:{{$gabung[$n][1]-15}}px; z-index:450;">
                ...
                @elseif($gabung[$n][7]=="end")
                    <svg id="pinstart" xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="-0.5 0 25 25" fill="blue" style="position: absolute; top:{{$gabung[$n][5]-30}}px; left:{{$gabung[$n][4]-15}}px; z-index:450;">
                        <path d="M3 11.3201C3 8.93312 3.94822 6.64394 5.63605 4.95612C7.32387 3.26829 9.61305 2.32007 12 2.32007C14.3869 2.32007 16.6762 3.26829 18.364 4.95612C20.0518 6.64394 21 8.93312 21 11.3201" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M3 11.3201C3 17.4201 9.76 22.3201 12 22.3201C14.24 22.3201 21 17.4201 21 11.3201" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M12 15.3201C14.2091 15.3201 16 13.5292 16 11.3201C16 9.11093 14.2091 7.32007 12 7.32007C9.79086 7.32007 8 9.11093 8 11.3201C8 13.5292 9.79086 15.3201 12 15.3201Z" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                @endif
                @if($n!=(count($gabung)-1))
                <button id="ruteTitiks" class="titikRute"  style="position: absolute; top:{{$gabung[$n][2]}}px; left:{{$gabung[$n][1]}}px; z-index:400;"><p>{{$gabung[$n][0]}}</p></button>
                <button id="ruteTitiks" class="titikRute"  style="position: absolute; top:{{$gabung[$n][5]}}px; left:{{$gabung[$n][4]}}px; z-index:400;"><p>{{$gabung[$n][3]}}</p></button>
                @else
                <button id="ruteTitiks" class="titikRute"  style="position: absolute; top:{{$gabung[$n][5]}}px; left:{{$gabung[$n][4]}}px; z-index:400;"><p>{{$gabung[$n][3]}}</p></button>
                @endif
            @endfor
            <svg class="lines" width="130vh" height="97vh" style="border:1px white solid; position:fixed; z-index:300;">
                @foreach($gabung as $line)
                    @if($line[6]=="green")
                    <line x1="{{(($line[1])-362)}}" y1="{{$line[2]-10}}" x2="{{(($line[4])-362)}}" y2="{{$line[5]-10}}" style="stroke: blue ;stroke-width:5;" />
                    @else
                    <line x1="{{(($line[1])-362)}}" y1="{{$line[2]-10}}" x2="{{(($line[4])-362)}}" y2="{{$line[5]-10}}" style="stroke: red ;stroke-width:2;" />
                    @endif
