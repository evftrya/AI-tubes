    ...
    public function store(Request $request)
{
    ...
    public function FindRute($tujuan,$Awal){
        ...
        // dd($datajrk,$shortestPaths);
        $backAll = [];
        for($m=0;$m<count($datajrk);$m++){
            array_push($backAll,array());
            $backAll[$m][0]=$datajrk[$m];
            $backAll[$m][1]=$shortestPaths[$m];
        }
        // dd($backAll);
    ...
    public function findShortestPaths($lines, $start, $end) {
        ...
        while (!empty($queue) && count($shortestPaths) < 7) {
